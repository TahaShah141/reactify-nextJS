...
export const HexColors: Record<string, Record<number, string>> = {
    "slate": {
        50: "#f8fafc",
        100: "#f1f5f9",
        200: "#e2e8f0",
...
export const getColorStyleType = (prefix: string, color: string, shade: number, CSSKey: string) : StyleType => {

    const tailwind = `${prefix}-${color}-${shade}`
...
export const getAllColorStyleTypes = (prefix: string, CSSKey: string): StyleType[] => {
  const toReturn: StyleType[] = []

  for (const shade of shades) {
    for (const color of colors) {
      toReturn.push(getColorStyleType(prefix, color, shade, CSSKey))
    }
  }
